...
GROUP BY gender
;
 #But first_name is not included in the GROUP BY clause, and it's not wrapped in an aggregate function like MAX(), MIN(), etc.
...
SELECT gender, MIN(age), MAX(age), COUNT(*), AVG(age)
FROM employee_demographics
GROUP BY gender;
